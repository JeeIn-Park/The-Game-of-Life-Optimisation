...
func calculateNextAliveCells(world [][]byte, start int, finish int) ([]util.Cell, []util.Cell) {
...
func (g *GameOfLifeOperation) EvaluateOne(req stubs.EvaluationRequest, res *stubs.AliveCells) (err error) {
	...
	if id == numberOfWorkers-1 {
		res.AliveCells, res.FlippedCells = calculateNextAliveCells(req.World, id*size, imageHeight)
	} else {
		res.AliveCells, res.FlippedCells = calculateNextAliveCells(req.World, id*size, (id+1)*size)
	}
	return
}
